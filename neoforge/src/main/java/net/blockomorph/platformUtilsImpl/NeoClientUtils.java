...
public class NeoClientUtils implements ClientPlatformUtils {
	...
	@Override
	public @Nullable TextureAtlasSprite[] getPlatformFluidSprite(BlockAndTintGetter lv, BlockState state, BlockPos keyPos) {
		return FluidSpriteCache.getFluidSprites(lv, keyPos, state.getFluidState());
	}

	@Override
	public Integer getPlatformFluidTint(BlockAndTintGetter lv, BlockState state, BlockPos keyPos) {
		return IClientFluidTypeExtensions.of(state.getFluidState()).getTintColor(state.getFluidState(), lv, keyPos);
	}
