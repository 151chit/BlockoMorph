...
public class TagAddingOverlay<T extends Tag> extends TagEditingOverlay {
	private static final Identifier MENU = GuiUtils.res("textures/screens/tag_adding_screen.png");
	private int tagBoxX;
	private int tagBoxY;
	...
	public void init(int width, int height, Consumer<TagEditingOverlay> onChange) {
		...
		this.name = new ListenerEditBox(GuiUtils.MC.font, this.leftPos + 7, this.topPos + 7, 89, 20, Component.literal("name"), s -> {}, ListenerEditBox.VANILLA);
		this.name.setFilter(this.filter);
		this.addRenderableWidget(this.name);
		this.name.setValue(nameValue);

		Button done = new Button.Plain(this.leftPos + 7, this.topPos + 32, 146, 20, CommonComponents.GUI_DONE, b -> {
			this.acceptTag();
		}, Supplier::get) {
			@Override
			protected void extractContents(GuiGraphicsExtractor GuiGraphicsExtractor, int mouseX, int mouseY, float tick) {
				this.isHovered = this.isMouseOver(mouseX, mouseY) && !TagAddingOverlay.this.selector.isListFocused(mouseX, mouseY);
				super.extractContents(GuiGraphicsExtractor, mouseX, mouseY, tick);
			}
		};
