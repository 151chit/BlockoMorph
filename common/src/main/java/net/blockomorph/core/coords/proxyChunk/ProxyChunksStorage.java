...
public class ProxyChunksStorage {
	...
	private void removeChunk(int chunkX, int chunkZ) {
		this.proxyChunks.remove(ChunkPos.pack(chunkX, chunkZ));
	}
	...
	private LevelChunk empty(long chunkPos) {
		return new EmptyLevelChunk(this.level, ChunkPos.unpack(chunkPos), this.level.registryAccess().lookupOrThrow(Registries.BIOME).getOrThrow(Biomes.PLAINS));
	}
