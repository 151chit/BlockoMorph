...
public class MorphNormalizer {
	...
	public static BlockPos normalizeSpawnpoint(BlockPos orig) {
		if (orig == null) return null;
		if (!InPlayerBlockPos.isMorphedPlayerBlockX(orig.getX())) return orig;
		PlayerAccessor pl = InPlayerBlockPos.findPlayer(orig);
		if (pl == null) return null;
		Vec3 normalized = normalize(Vec3.atBottomCenterOf(orig));
		return BlockPos.containing(normalized);
	}
