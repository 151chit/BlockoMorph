...
public class AllPlayersBlockPoses {
	...
	private final Object2LongMap<UUID> allPlayers = new Object2LongOpenHashMap<>();
	...
	protected void load() {
		...
		try {
			...
			tag.forEach((uuidKey, sectionTag) -> {
				if (sectionTag instanceof LongTag(long value)) {
					try {
						UUID uuid = UUID.fromString(uuidKey);
						this.allPlayers.put(uuid, value);
					} catch (Exception e) {
