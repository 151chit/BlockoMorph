...
public class BlockEntityAndEntityIO {
	...
	public List<String> loadInBlockEntity(BlockEntity blockEntity, RegistryAccess access, CompoundTag tag) {
		try {
			...
			blockEntity.loadWithComponents(tag, access);
	...
	public ResultAndErrors<CompoundTag> saveBlockEntity(BlockEntity blockEntity, RegistryAccess access) {
		...
		try {
			this.setReporterActive(true);
			compoundTag = blockEntity.saveWithoutMetadata(access);
		} catch (Exception e) {
			this.handleMainError(e);
			compoundTag = new CompoundTag();
		} finally {
			this.setReporterActive(false);
		}
		return new ResultAndErrors<>(compoundTag, this.flushProblems());
	}

	@Nullable
	public List<String> loadInEntity(Entity entity, RegistryAccess ignored, CompoundTag tag) {
		try {
			this.setReporterActive(true);
			entity.load(tag);
		} catch (Exception e) { this.handleMainError(e); } finally {
	...
	public ResultAndErrors<Entity> makeEntityFromTag(Level level, CompoundTag tag) {
		try {
			this.setReporterActive(true);
			Optional<Entity> entity = EntityType.create(tag, level, EntitySpawnReason.LOAD);
			if (entity.isPresent()) {
	...
	public ResultAndErrors<CompoundTag> saveEntity(Entity entity, RegistryAccess ignored) {
