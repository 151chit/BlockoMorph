...
public class AsyncBlocksRenderer extends BakedBlocksRenderer {
	...
	private void drawInternal(PlayerSectionLayerGroup group) {
		if (this.hasOld && this.state != AsyncState.OFF) {
			...
			GpuBufferSlice dynamicTransforms = RenderSystem.getDynamicUniforms()
					.writeTransform(RenderSystem.getModelViewMatrix(), COLOR_MODULATOR, this.playerPosOffset, TEXTURE_MATRIX, RenderSystem.getShaderLineWidth());
			try (RenderPass renderPass = RenderSystem.getDevice().createCommandEncoder()
				...
				for (PlayerSectionLayer layer : group.layers()) {
					...
					renderPass.bindSampler("Sampler0", blockAtlas.getTextureView());
					renderPass.bindSampler("Sampler2", GuiUtils.MC.gameRenderer.lightTexture().getTextureView());
	...
	private static RenderPipeline selectPipeline(PlayerSectionLayer layer) {
		return switch (layer) {
			case SOLID -> RenderPipelines.SOLID;
			case CUTOUT -> RenderPipelines.CUTOUT;
			case TRANSLUCENT -> RenderPipelines.TRANSLUCENT;
		};
	}
