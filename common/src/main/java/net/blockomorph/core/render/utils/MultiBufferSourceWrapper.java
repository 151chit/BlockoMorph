...
public class MultiBufferSourceWrapper implements SortedRenderOutput {//todo sort
	...
	public void appendBreaks(PoseStack poseStack, BlockState block, long seed, int progress) {
		BakedModel model = GuiUtils.MC.getModelManager().getBlockModelShaper().getBlockModel(block);
		...
		GuiUtils.MC.getBlockRenderer().getModelRenderer().renderModel(poseStack.last(), vertexConsumer, null, model, 1.0F, 1.0F, 1.0F, LightTexture.FULL_BRIGHT, OverlayTexture.NO_OVERLAY);
	}

	@Override
	public <T> void appendEntityObject(T gameObjectOrState, PoseStack stack) {
		if (gameObjectOrState instanceof ContextDependStateExtractor.TntStateWithRenderer<?, ?> tntRenderState) {
	...
	private <S extends TntRenderState, R extends TntRenderer> void submitTnt(ContextDependStateExtractor.TntStateWithRenderer<S, R> stateWithRenderer, PoseStack poseStack) {
		if (GuiUtils.MC.level == null) return;
		TntRenderState state = stateWithRenderer.tntRenderState();
		MUTABLE_BLOCK_POS.set(state.x, state.y, state.z);
		try {
			int light = LevelRenderer.getLightColor(GuiUtils.MC.level, MUTABLE_BLOCK_POS);
			stateWithRenderer.renderer().render(state, poseStack, this.multiBufferSource.get(), light);
