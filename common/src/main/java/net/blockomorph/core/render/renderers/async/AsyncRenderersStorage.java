...
public class AsyncRenderersStorage {
	...
	private boolean shouldRender(Frustum frustum) {
		...
		boolean base = frustum == null || entityRenderer.shouldRender(
				this.player.player(), frustum, camera.getPosition().x, camera.getPosition().y, camera.getPosition().z) ||
				this.player.player().hasIndirectPassenger(localPlayer);
		if (!base) return false;

		boolean camCondition = this.player != camera.getEntity() || camera.isDetached() || camera.getEntity() instanceof LivingEntity lv && lv.isSleeping();
		if (!camCondition) return false;

		return !(this.player instanceof LocalPlayer) || camera.getEntity() == this.player || this.player == localPlayer && !localPlayer.isSpectator();
	}
