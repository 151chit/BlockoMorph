...
public class PlayerMorphedSection {
	...
	public static Long2LongMap absoluteToOffset(PlayerAccessor pl) {
		...
		for (int x = 1; x <= chunksCount; x++) {
			for (int z = 1; z <= chunksCount; z++) {
				long absolute = ChunkPos.asLong(SectionPos.blockToSectionCoord(blockX + x*8), SectionPos.blockToSectionCoord(blockZ + z*8));
				long offset = ChunkPos.asLong(SectionPos.blockToSectionCoord(x*8), SectionPos.blockToSectionCoord(z*8));
