...
public class DamageHandler {
			...
			if (noActiveTnt) {
				...
				if (tnt != null) for (ResourceKey<DamageType> key : TNT_DAMAGE) {
					if (damage.typeHolder().is(key)) {
						int i = tnt.getFuse();
						tnt.setFuse(pl.player().level().getRandom().nextInt(i / 4) + i / 8);
						break;
					}
				}
			}
	...
	private static boolean allowDamage(DamageSource damage, float hp, Set<ResourceKey<DamageType>> need) {
		Map<Identifier, Integer> allowed = Config.get().allowedDamages.getValue();
		for (ResourceKey<DamageType> key : need) {
			if (damage.is(key) && allowed.containsKey(key.identifier()) && hp >= allowed.get(key.identifier())) return true;
		}
		return false;
	}
	...
	public static void destroy(PlayerAccessor morphed, @Nullable Entity attacker) {
		...
		if (morphed instanceof ServerPlayer pl && pl.connection.hasClientLoaded() && pl instanceof ServerPlayerAccessor acc) {
			try {
