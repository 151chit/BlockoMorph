...
public abstract class RegistryMapIdsConfig<VALUE, REG> extends ConfigInstance<Map<ResourceLocation, VALUE>> {
	...
	protected RegistryMapIdsConfig(String name, Map<ResourceLocation, VALUE> initialValue, boolean canOperatorModify, @Nullable Component tip, RegistryMapOptionContext ctx, ResourceKey<Registry<REG>> registryId) {
	...
	public void readFromStorage(JsonElement option) {
		...
		option.getAsJsonObject().asMap().forEach((key, jsonElement) -> {
			ResourceLocation raw = ResourceLocation.tryParse(key);
	...
	public void parseFromUser(ServerPlayer ctx, String value) {
		...
		ResourceLocation KEY = ResourceLocation.tryParse(key);
	...
	public LiteralArgumentBuilder<CommandSourceStack> buildArgument(LiteralArgumentBuilder<CommandSourceStack> optionNameArgument, CommandBuildContext context, Commands.CommandSelection environment) {
		Map.Entry<Set<ResourceLocation>, Boolean> preparedAddCensorListHolder = Objects.requireNonNullElseGet(this.context.addCensorList, () -> Map.entry(Set.of(), true));
		Map.Entry<Set<ResourceLocation>, Boolean> preparedRemoveCensorListHolder = Objects.requireNonNullElseGet(this.context.removeCensorList, () -> Map.entry(Set.of(), true));
		return optionNameArgument
				.then(Commands.literal("set").then(Commands.argument("key", FilteredResourceArgument.id(context, this.registryId, preparedAddCensorListHolder.getKey(), preparedAddCensorListHolder.getValue())).then(Commands.argument("value", this.getValueArgument(context, environment)).executes(args -> {
					...
					ResourceLocation name = FilteredResourceArgument.getId(args, "key", this.registryId).location();
				...
				.then(Commands.literal("getCollector").then(Commands.argument("key", ResourceArgument.resource(context, this.registryId)).executes(args -> {
					ResourceLocation name = ResourceArgument.getResource(args, "key", this.registryId).key().location();
	...
	public record RegistryMapOptionContext(boolean canClear, @Nullable Map.Entry<Set<ResourceLocation>, Boolean> addCensorList, @Nullable Map.Entry<Set<ResourceLocation>, Boolean> removeCensorList) {
	}
}
