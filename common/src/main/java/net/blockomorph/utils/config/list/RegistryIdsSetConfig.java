...
public abstract class RegistryIdsSetConfig<T> extends ConfigInstance<Set<ResourceLocation>> {
	protected final IdsSetOptionContext context;
	protected final ResourceKey<Registry<T>> registryId;
	protected RegistryIdsSetConfig(String name, Set<ResourceLocation> initialValue, boolean canOperatorModify, @Nullable Component tip, IdsSetOptionContext context, ResourceKey<Registry<T>> registryId) {
	...
	public void parseFromUser(ServerPlayer ctx, String value) {
		...
		ResourceLocation ID = new ResourceLocation(id);
	...
	private LiteralArgumentBuilder<CommandSourceStack> end(LiteralArgumentBuilder<CommandSourceStack> actionString, boolean remove, CommandBuildContext ctx) {
		Map.Entry<Set<ResourceLocation>, Boolean> censorListHolder = remove ? this.context.removeCensorList : this.context.addCensorList;
		Map.Entry<Set<ResourceLocation>, Boolean> preparedCensorListHolder = Objects.requireNonNullElseGet(censorListHolder, () -> Map.entry(Set.of(), true));
		return actionString.then(Commands.argument("value", FilteredResourceArgument.id(ctx, this.registryId, preparedCensorListHolder.getKey(), preparedCensorListHolder.getValue())).executes(args -> {
			ResourceLocation name = FilteredResourceArgument.getId(args, "value", this.registryId).location();
			Component end;
			if (remove) {
	...
	public record IdsSetOptionContext(boolean canClear, @Nullable Map.Entry<Set<ResourceLocation>, Boolean> addCensorList, @Nullable Map.Entry<Set<ResourceLocation>, Boolean> removeCensorList) {
