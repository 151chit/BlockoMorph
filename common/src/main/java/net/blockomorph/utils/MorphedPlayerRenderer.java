...
public class MorphedPlayerRenderer {
	...
	public boolean render(AbstractClientPlayer player, float partialTicks, PoseStack posestack, MultiBufferSource buffer, int light, Consumer<Float> shadow) {
		if (player instanceof PlayerAccessor pl) {
			if (pl.isFullActive()) {
				...
				this.renderTnt(partialTicks, posestack, buffer, light, pl);
				return true;
			}
		}
		return false;
	...
	private void renderTnt(float partialTicks, PoseStack posestack, MultiBufferSource buffer, int light, PlayerAccessor pl) {
		PrimedTnt tnt = pl.getTnt();
		EntityRenderer<? super PrimedTnt, ?> rend = MC.getEntityRenderDispatcher().getRenderer(tnt);
		try {
			this.renderTntInternal(rend, tnt, partialTicks, posestack, buffer, light);
		} catch (Exception ignored) {
		}
	}

	private <S extends EntityRenderState> void renderTntInternal(EntityRenderer<? super PrimedTnt, S> renderer, PrimedTnt tnt, float partialTicks, PoseStack posestack, MultiBufferSource buffer, int light) {
		S state = renderer.createRenderState(tnt, partialTicks);
		renderer.render(state, posestack, buffer, light);
	}
	...
	private void renderLiquid(boolean translucent, BlockInPlayer2 block, PoseStack posestack, MultiBufferSource buffer, PlayerAccessor pl) {
		...
		if (!fluidState.isEmpty()) {
			...
			switch (layer) {
				case CUTOUT_MIPPED -> renderType = RenderType.cutoutMipped();
				case CUTOUT -> renderType = RenderType.cutout();
				case TRANSLUCENT -> renderType = RenderType.translucentMovingBlock();
				case TRIPWIRE -> renderType = RenderType.tripwire();
