...
public class FilteredResourceArgument<REGISTRY> implements ArgumentType<ResourceKey<REGISTRY>> {
	...
	public static class ContextInfo<T> implements MorphUtils.ArgumentEncoder<FilteredResourceArgument<T>, ContextInfo<T>.Template> {
		...
		public Template deserializeFromNetwork(FriendlyByteBuf buf) {
			...
			return new Template(key, bannedValues, blackList);
		}
		...
		public Template unpack(FilteredResourceArgument<T> argument) {
			return new Template(argument.registryId, argument.censorList, argument.blackList);
		}
