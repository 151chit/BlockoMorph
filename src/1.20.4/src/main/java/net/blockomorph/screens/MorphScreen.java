...
public class MorphScreen extends Screen {
	...
	private static final CreativeModeTab search = CreativeModeTabRegistry.getTab(new ResourceLocation("search"));
	private static final CreativeModeTab op_tab = CreativeModeTabRegistry.getTab(new ResourceLocation("op_blocks"));
	private static final CreativeModeTab loved_blocks = CreativeModeTabRegistry.getTab(new ResourceLocation("hotbar"));
    ...
    private static final WidgetSprites DEMORPH_BUT = new WidgetSprites(
   	 new ResourceLocation("blockomorph", "textures/screens/demorph_def.png"),
   	 new ResourceLocation("blockomorph", "textures/screens/demorph_dis.png"),
   	 new ResourceLocation("blockomorph", "textures/screens/demorph_hov.png")
    );
    ...
    protected void renderTabButton(GuiGraphics gui, CreativeModeTab tab, int i, boolean isLeft) {
        ...
        gui.blit(new ResourceLocation("textures/gui/sprites/advancements/tab_" + tabType + ".png"), l, i1, 0, 0, weight, height, weight, height);
			...
			if (i != -1) {
				...
				if (!this.isConfig()) {
					if (MorphUtils.isBannedBlock(st).isEmpty()) {
				        PacketDistributor.SERVER.noArg().send(ServerBoundBlockMorphPacket.create(st, tg));
					}
				} else if (this.mode == Config.Mode.WHITELIST) {
	...
	public void init() {
		...
		this.unmask = new NonSpritedImageButton(this.leftPos + 10, this.topPos + this.imageHeight + 1, 26, 26, DEMORPH_BUT, e -> {
			 PacketDistributor.SERVER.noArg().send(ServerBoundBlockMorphPacket.create(Blocks.AIR.defaultBlockState(), new CompoundTag()));
		});
