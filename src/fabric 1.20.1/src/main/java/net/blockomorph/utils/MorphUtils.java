   	        ...
   	        if (mc.player != null) {
                ...
                if (hitEntity instanceof PlayerAccessor pl && pl.isActive()) {
                	...
                	if ((hitPart != null && !isAttackPressed && i > -1) || (attackPressed && !isAttackPressed)) {
                        ClientPlayNetworking.send(MainBus.INTERACT_PACKET, new ServerBoundInteractBlockPacket(false, -1, hitPart));
                        pl.setReady(true);
                	}
                    ...
                    if (isAttackPressed && pl.readyForDestroy()) {
                    	...
                    	if (gm.getDelay() > 0) {
                    		...
                    		ClientPlayNetworking.send(MainBus.INTERACT_PACKET, new ServerBoundInteractBlockPacket(true, hitEntity.getId(), hitPart));
